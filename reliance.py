...
class RelianceStockAnalyzer:
    ...
    def fetch_stock_data(self, start_date: str, end_date: str) -> None:
        ...
        try:
            self.stock_data = yf.download('RELIANCE.NS', start=start_date, end=end_date)
    ...
    def calculate_rsi(self, window: int = 14) -> pd.Series:
        ...
        self.rsi = pd.Series(rsi, index=self.stock_data.index)
        return self.rsi
...
def main():
    ...
    try:
        ...
        analyzer.fetch_stock_data(start_date.strftime('%Y-%m-%d'), end_date.strftime('%Y-%m-%d'))
        ...
        analyzer.stock_data['Close'].plot(ax=ax2, label='Close Price', color='black')
        ...
        analyzer.signal.plot(ax=ax3, label='Signal Line', color='red')
        ...
        analyzer.rsi.plot(ax=ax_rsi, color='purple')
